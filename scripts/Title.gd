...
func _ready():
	...
	get_node("GameOver/VBoxContainer/Score").text += str(Info.rightAction) + "\n"
	if(Info.image != null):
...
func _input(event):
	...
	) and $VBoxContainer.visible == true:
		$VBoxContainer/MarginContainer/ScrollContainer.scroll_vertical -= (
			event.relative.y)
...
func _on_Zen_pressed():
	tap()
	Info.arcade = false
	get_node("Gamemode/MC/RichTextLabel"
	).text = "Zen: lose after first mistake. Score equals to number of profiles looked through multiplied by number of requests"
...
func _on_IconHeart_pressed():
	...
	get_node("MarginContainer/VBoxContainer/CC/IconHeart").icon.current_frame = (
		get_node("MarginContainer/VBoxContainer/CC/IconHeart").icon.current_frame + 1) % 2
